...
public class StudentService {
    ...
    public boolean registerForCourse(Student student, String courseCode) throws CourseFullException {
        Course course = courseService.findCourseByCode(courseCode);
        if (course != null && course.isFull()) {
            throw new CourseFullException("Registration failed because the course enrollment limit has been reached.");
        }
    ...
    public boolean dropCourse(Student student, String courseCode) throws DropDeadlinePassedException {
        ...
        if (enrollment.isDropDeadlinePassed()) {
            throw new DropDeadlinePassedException(
                    "Course drop failed because the drop deadline has already passed for " + courseCode + ".");
        }
    ...
    public Feedback<Integer> submitNumericFeedback(Student student, String courseCode, int rating) {
        return feedbackService.submitNumericFeedback(student, courseCode, rating);
    }

    public Feedback<String> submitTextFeedback(Student student, String courseCode, String comment) {
        return feedbackService.submitTextFeedback(student, courseCode, comment);
    }
}
